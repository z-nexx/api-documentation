...
#### Errors
When using an expired access token in an API request, the resource service will return an error in the following format:
```
HTTP 401 Unauthorized
{
    "meta": {
        "developerMessage": "Access token expired at: NumericDate{1472482768 -> Aug 29, 2016 2:59:28 PM UTC}",
        "errorCode": "ACCESS_TOKEN_EXPIRED"
    }
}
```

Additionally, the HTTP header `WWW-Authenticate` will contain similar information:
```
Bearer error="invalid_request", error_description="The access token has expired"
``` 
