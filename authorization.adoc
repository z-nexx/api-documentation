...
#### Errors
When using an expired access token in an API request, the resource service will return any error with the token in the HTTP header `WWW-Authenticate`, similar to this:
```
